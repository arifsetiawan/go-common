...
// See: `Logger()`.
func LoggerWithConfig(config LoggerConfig, appName string) echo.MiddlewareFunc {
	// Defaults
		...
		return func(c echo.Context) (err error) {
			...
			if _, err = config.template.ExecuteFunc(buf, func(w io.Writer, tag string) (int, error) {
				...
				case "tenant":
					...
					return buf.WriteString(tenant)
				case "app":
					return buf.WriteString(appName)
				case "type":
					return buf.WriteString("http")
				case "level":
